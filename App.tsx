...
import { Article, Category, UserPreferences, MonetizationConfig, Video, WalletState, Transaction, UserProfile } from './types.ts';
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-300 relative ${isDark ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-900'}`}>
        ...
        <div className="max-w-[1600px] mx-auto px-4 h-16 sm:h-20 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <button 
              ...
              className={`flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full font-black text-[10px] sm:text-xs uppercase tracking-widest transition-all border group relative overflow-hidden ${
                isDark 
                  ? 'border-blue-500/60 text-blue-400 bg-blue-900/30' 
            ...
            {isAuthenticated && (
              <button 
                ...
                className="p-2 hover:bg-red-50 hover:text-red-600 rounded-full transition-colors hidden sm:block"
                title="Logout"
              >
                <LogOut size={20} />
              </button>
            ...
            <button 
              ...
              className={`flex items-center gap-2 p-1 sm:pr-3 rounded-full border transition-all ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}
            >
              <div className={`w-8 h-8 rounded-full ${userProfile?.avatarColor || 'bg-slate-200'} flex items-center justify-center text-white font-black text-xs`}>
                {userProfile?.name?.charAt(0) || <UserIcon size={16} className="text-slate-400" />}
              </div>
        ...
        {showAdminDashboard ? (
          ...
          <div className="animate-in fade-in duration-500">
            <div className="flex items-center justify-between mb-8">
               <h2 className="text-2xl sm:text-4xl font-black font-serif flex items-center gap-3">
                 {selectedCategory}
                 {selectedCategory === 'Bookmarks' && <span className="text-xs font-sans font-bold text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">{bookmarks.length}</span>}
               </h2>
            ...
            {currentArticles.length > 0 ? (
                ...
                {totalPages > 1 && (
                  <div className="mt-16 flex justify-center items-center gap-2 sm:gap-4">
                    <button 
                      ...
                      className="p-2 sm:p-3 rounded-full border border-slate-200 dark:border-slate-800 disabled:opacity-30 hover:bg-blue-50 dark:hover:bg-slate-800 transition-all"
                    >
                      <ChevronLeft size={20} />
                    </button>
                    <div className="flex gap-1 sm:gap-2 overflow-x-auto no-scrollbar max-w-[200px] sm:max-w-none">
                       {Array.from({ length: totalPages }).map((_, i) => (
                         <button
                           ...
                           className={`min-w-[32px] sm:min-w-[40px] h-8 sm:h-10 rounded-full font-bold text-[10px] sm:text-sm transition-all ${currentPage === i + 1 ? 'bg-blue-600 text-white shadow-lg' : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500'}`}
                         >
                           {i + 1}
                         </button>
                       ))}
              ...
              <div className="py-20 text-center">
                 <h3 className="text-xl font-bold text-slate-400">No stories found.</h3>
              </div>
            )}
          </div>
        )}
         ...
         <div className="max-w-[1600px] mx-auto px-4 flex flex-col items-center gap-8">
            ...
            <div className={`w-full max-w-4xl border-b ${isDark ? 'border-slate-800' : 'border-slate-100'} pb-10 mb-2`}>
               ...
               <div className="flex flex-wrap justify-center items-center gap-x-10 gap-y-6 md:gap-x-16 opacity-40 grayscale hover:grayscale-0 transition-all duration-700">
                  <div className="flex items-center gap-2 font-black text-xs tracking-tighter">
                    <div className="w-7 h-7 bg-blue-600 rounded-lg flex items-center justify-center text-[10px] text-white shadow-sm font-sans">M</div>
                    MOVA AI
                  </div>
                  <div className="flex items-center gap-2 font-black text-xs tracking-tighter">
                    <div className={`w-7 h-7 ${isDark ? 'bg-white text-slate-900' : 'bg-slate-900 text-white'} rounded-lg flex items-center justify-center text-[10px] font-serif shadow-sm`}>G</div>
                    GLOBAL PRESS
                  </div>
                  <div className="flex items-center gap-2 font-black text-xs tracking-tighter">
                    <div className="w-7 h-7 bg-emerald-500 rounded-lg flex items-center justify-center text-[10px] text-white italic font-serif shadow-sm">T</div>
                    TECHFLOW
                  </div>
                  <div className="flex items-center gap-2 font-black text-xs tracking-tighter">
                    <div className="w-7 h-7 bg-rose-600 rounded-lg flex items-center justify-center text-[10px] text-white shadow-sm">S</div>
                    SPORTSGRID
                  </div>
                  <div className="flex items-center gap-2 font-black text-xs tracking-tighter">
                    <div className="w-7 h-7 bg-indigo-600 rounded-lg flex items-center justify-center text-[10px] text-white shadow-sm font-sans italic">P</div>
                    PURE INSIGHT
                  </div>
               </div>
            </div>
      ...
      <UserDashboard 
        ...
        stats={{ articlesRead: 24, bookmarks: bookmarks.length, comments: 4 }}
      />
