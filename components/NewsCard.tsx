...
import { ArrowRight, Clock, Hash, Eye, Heart, MessageSquare, ShieldCheck } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="relative w-full aspect-video overflow-hidden bg-slate-200 dark:bg-slate-700">
        ...
        {/* Branded News Banner (Requested Feature) */}
        {article.subject && (
           <div className="absolute bottom-0 left-0 right-0 z-20">
              <div className="bg-slate-900/90 backdrop-blur-sm border-t border-white/10 px-4 py-2.5 flex items-center gap-3">
                 <div className="bg-blue-600 text-white p-1 rounded-sm shadow-sm flex-shrink-0">
                    <ShieldCheck size={12} />
                 </div>
                 <p className="text-white text-[10px] sm:text-xs font-black uppercase tracking-widest line-clamp-1 italic">
                    {article.subject}
                 </p>
              </div>
           </div>
        ...
        {/* Content Preview Overlay (Appears on Hover) */}
        ...
        <div className="absolute bottom-3 right-3 opacity-30 z-20 pointer-events-none scale-75 origin-bottom-right group-hover:opacity-0 transition-opacity">
        ...
        <div className={`mt-auto pt-4 border-t flex items-center justify-between font-semibold text-sm ${
          ...
          <div className="flex items-center gap-2">
            ...
            <div className={`flex items-center gap-1.5 p-2 ${isDark ? 'text-slate-400' : 'text-slate-500'}`} title="Comments">
              ...
              <span className="text-xs">{article.userComments?.length || 0}</span>
            </div>
          </div>

          <div className={`flex items-center ${isDark ? 'text-blue-400 group-hover:text-blue-300' : 'text-blue-600 group-hover:text-blue-700'}`}>
            <span className="relative text-xs sm:text-sm font-bold uppercase tracking-wide">
              Read Story
