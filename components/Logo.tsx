...
const Logo: React.FC<LogoProps> = ({ className = "", isDark = false }) => {
  return (
    <div className={`flex items-center gap-2 select-none ${className}`}>
      <div className={`relative flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-lg shadow-sm ${isDark ? 'bg-blue-600' : 'bg-slate-900'} transition-colors`}>
